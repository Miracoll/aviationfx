...
def updateUser(request, ref):
    ...
    if 'updatebalance' in request.POST:
        ...
        # Send mail to user
        subject = 'AVIATIONFX UPADTE'
        recipient = [act_user.email]
        text_content = f'Dear {act_user.last_name} your email request token is {token}'
        html_content = f'<div><h3 style="color:purple">AVIATIONFX</h3></div><div><p>Dear {act_user.last_name} your email request token is {token}</div>'
        ...
        html_content = f'<div><h3 style="color:purple">AVIATIONFX</h3></div><div><p>Dear {act_user.last_name} your withdrawal request token is {token}</div>'
        message = EmailMultiAlternatives(subject=subject, body=text_content, to=recipient)
        message.attach_alternative(html_content, 'text/html')
        message.send()
        messages.success(request, 'Token sent')
        ...
        html_content = f'<div><h3 style="color:purple">AVIATIONFX</h3></div><div><p>Dear {act_user.last_name}, <br>{msg}</div>'
        message = EmailMultiAlternatives(subject=subject, body=text_content, to=recipient)
        message.attach_alternative(html_content, 'text/html')
        message.send()
...
def paymentApprove(request, ref):
    ...
    # html_content = f'<div><h3 style="color:purple">AVIATIONFX</h3></div><div><p>Dear {act_user.last_name}, <br>Your payment has been approved</div>'
    # message = EmailMultiAlternatives(subject=subject, body=text_content, to=recipient)
    # message.attach_alternative(html_content, 'text/html')
    # message.send()
    # messages.success(request, 'Message sent')
...
def addCrypto(request):
    if request.method == 'POST':
        ...
        if coin == 'crypto':
            url = f'https://fcsapi.com/api-v3/{coin}/list?type={coin}&access_key=ITDL40JdWn3ER97wAGRh9'
        ...
        if coin == 'forex':
            ...
            return redirect('admin-set-coin')
        if coin == 'stock':
            url = f'https://fcsapi.com/api-v3/stock/list?country=United-states&access_key=ITDL40JdWn3ER97wAGRh9'
            r = requests.get(url = url)
    ...
    if request.method == 'POST':
        ...
        if coin == 'crypto':
            ...
            for i in currency.values():
                print(i)
                print('----------------------------------------------------')
                url = f"https://fcsapi.com/api-v3/crypto/profile?symbol={i.get('symbol')}&access_key=ITDL40JdWn3ER97wAGRh9"
                r = requests.get(url = url)
                data = r.json()
                extract_data = data.get('response')
                print(extract_data)
        ...
        if coin == 'forex':
            ...
            print(total)
            for i in currency.values():
                url = f"https://fcsapi.com/api-v3/forex/profile?id={i.get('code')}&access_key=ITDL40JdWn3ER97wAGRh9"
                r = requests.get(url = url)
                data = r.json()
                extract_data = data.get('response')
...
def setCoin(request):
    ...
    for i in apilist:
        if i == 'crypto':
            url = f'https://fcsapi.com/api-v3/forex/list?type={i}&access_key=ITDL40JdWn3ER97wAGRh9'
            r = requests.get(url = url)
            data = r.json()
            extract_data = data.get('response')
            ...
            url = f'https://fcsapi.com/api-v3/stock/list?country=United-states,Japan&access_key=ITDL40JdWn3ER97wAGRh9'
            r = requests.get(url = url)
            data = r.json()
            extract_data = data.get('response')
    ...
    if 'country' in request.POST:
        ...
        url = f'https://fcsapi.com/api-v3/stock/list?country={cnt}&access_key=ITDL40JdWn3ER97wAGRh9'
        r = requests.get(url = url)
...
def setCryptoAdd(request, ref):
    ...
    if NewCoin.objects.filter(name=currency.name, type='crypto').exists():
        ...
        url = f"https://fcsapi.com/api-v3/crypto/profile?symbol={currency.symbol}&access_key=ITDL40JdWn3ER97wAGRh9"
        r = requests.get(url = url)
        data = r.json()
        extract_data = data.get('response')
        if len(extract_data) < 2:
...
def setForexAdd(request, ref):
    ...
    if NewCoin.objects.filter(name=currency.name, type='forex').exists():
        ...
        url = f"https://fcsapi.com/api-v3/forex/profile?id={currency.code}&access_key=ITDL40JdWn3ER97wAGRh9"
        r = requests.get(url = url)
        data = r.json()
        extract_data = data.get('response')
        for j in extract_data:
